...
//
// This file is based on $GOROOT/misc/wasm/wasm_exec.js with modifications
// to work as an ES module.
...
// Setup performance
if (!globalRef.performance) {
	if (isNodeJS) {
		const { performance } = await import("perf_hooks");
		globalRef.performance = performance;
	}
}
...
if (isNodeJS) {
	globalRef.fs = {
		...
		close(fd, callback) { callback(new Error("not implemented")); },
		...
		read(fd, buffer, offset, length, position, callback) { callback(new Error("not implemented")); },
		...
		this.importObject = {
			...
			gojs: {
				...
				"runtime.wasmWrite": (sp) => {
					...
					globalRef.fs.writeSync(fd, new Uint8Array(this._inst.exports.mem.buffer, p, n));
				},
